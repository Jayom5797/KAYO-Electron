 ...
 *
 * This is the single shared security core used by every front-end (Electron,
 * CLI, and the web dashboard) so they all produce identical findings. Network-
 * touching modules (TLS, DNS) run under Promise.allSettled so one failing probe
 * never sinks the whole analysis.
 */
export async function analyzeSecurity(
  ...
  // CVE correlation for fingerprinted components with known versions (best-effort).
  ...
  if (fingerprintValue) {
    try {
      ...
      /* CVE lookup is best-effort */
    }
  }

  // The active scan is INTRUSIVE and only runs on explicit, authorized opt-in.
  const vuln =
    options.activeScan === true
      ? await runVulnScan(api).catch(() => EMPTY_VULN)
  ...
  return {
    tls:
      tlsResult.status === 'fulfilled'
        ? tlsResult.value
        : { error: (tlsResult.reason as Error).message },
...
/**
 * End-to-end scan: validate → capture → metrics → HAR → full security analysis.
...
export async function runScan(rawUrl: string, options: ScanOptions = {}) {
  ...
  const security = await analyzeSecurity(validation.url, capture.requests, options);
